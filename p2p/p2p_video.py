...
# ldm_stable = StableDiffusionPipeline.from_pretrained(MODEL).to(device)
...
class AttentionControl(abc.ABC):
    ...
    def __call__(self, attn, is_cross: bool, place_in_unet: str):
        ...
        if self.cur_att_layer == self.num_att_layers + self.num_uncond_att_layers:
            ...
            self.between_steps()
        # else:
        #     print(f"cur:{self.cur_att_layer},num: {self.num_att_layers}, uncon: {self.num_uncond_att_layers}")
...
class AttentionStore(AttentionControl):
        ...
        if len(self.attention_store) == 0:
            ...
            for key in self.attention_store:
                for i in range(len(self.attention_store[key])):
                    print(i , len(self.step_store), len(self.attention_store))
    ...
    def __init__(self, tokenizer, device=0):
...
class AttentionControlEdit(AttentionStore, abc.ABC):
    ...
    def __init__(self, tokenizer, prompts, num_steps: int,
                 ...
                 local_blend: Optional[LocalBlend], device: int=0):
        super(AttentionControlEdit, self).__init__(tokenizer, device)
        self.device = device
...
class AttentionReplace(AttentionControlEdit):
    ...
    def replace_cross_attention(self, attn_base, att_replace):
        self.mapper = self.mapper.to(attn_base.dtype).to(attn_base.device)
        return torch.einsum('hpw,bwn->bhpn', attn_base, self.mapper)
      
    def __init__(self, tokenizer, prompts, num_steps: int, cross_replace_steps: float, self_replace_steps: float,
                 local_blend: Optional[LocalBlend] = None, device: int=0):
