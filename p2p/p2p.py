...
class LocalBlend:
    ...
    def __call__(self, x_t, attention_store, ref_attention_store):
        ...
        if self.counter > self.start_blend:
            ...
            mask = Image.open("stylize/warp_test/50_densepose_384_mask_dilate.png").resize((width, height))
            mask = np.array(mask).astype(np.float32) / 255.0
            if len(mask.shape) == 3:
                mask = mask[:,:,0]
            mask = torch.from_numpy(mask).unsqueeze(0).unsqueeze(0).repeat(batchsize, 1, 1, 1)
