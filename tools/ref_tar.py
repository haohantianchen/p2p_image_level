...
# 读取两个CSV文件
df1 = pd.read_csv('/raid/cvg_data/ECCV2024/code/vid2densepose/csv/00000050.csv')
df2 = pd.read_csv('/raid/cvg_data/ECCV2024/code/vid2densepose/csv/00000000.csv')

tensor1 = torch.tensor(df1[df1['i'] != 0].values,dtype=torch.float16)
print(tensor1)
print(tensor1.shape)
tensor2 = torch.tensor(df2[df2['i'] != 0].values,dtype=torch.float16)
print(tensor2)
print(tensor2.shape)
...
for i in tqdm(range(shape1[0]), desc="Processing", unit="row"):
    ...
    # 将结果保存到result_tensor
    if distances[min_index].unsqueeze(0) < 0.01 :
...
# 保存为 CSV 文件
df_int.to_csv('/raid/cvg_data/lurenjie/P2P/prompt_travel/stylize/warp_test/the_0_50.csv', index=False)

# 读取CSV文件
df = pd.read_csv("/raid/cvg_data/lurenjie/P2P/prompt_travel/stylize/warp_test/the_0_50.csv")
...
# 保存结果到新的CSV文件
unique_rows.to_csv("/raid/cvg_data/lurenjie/P2P/prompt_travel/stylize/warp_test/the_0_50_mean.csv", index=False)