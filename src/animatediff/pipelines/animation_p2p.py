...
class MyAnimationPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # latents = latents[:,:, 0:1, :, :].repeat(1, 1, video_length, 1, 1)

        if init_latents is not None:
            latents = init_latents * self.scheduler.init_noise_sigma
            latents = latents.expand(video_length, self.unet.in_channels, height // 8, width // 8)
            latents = rearrange(latents, "(b f) c h w -> b c f h w", f=video_length)
        if uncondition_embeddings is not None:
            prompt_embeds_list = []
            for unc in uncondition_embeddings:
                unc = unc.repeat(video_length, 1, 1).to(self.device)
...
#            self.prepare_controlnet_ref_only(
        ...
        with self.progress_bar(total=total_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if num_inverse_steps != -1 and i == num_inverse_steps:
                    ...
                    if motion_module_path.exists() and motion_module_path.is_file():
                        ...
                        if motion_module_path.suffix.lower() in [".pth", ".pt", ".ckpt"]:
                            motion_state_dict = torch.load(motion_module_path, map_location=self.device, weights_only=True)
                        elif motion_module_path.suffix.lower() == ".safetensors":
                            motion_state_dict = load_file(motion_module_path, device=self.device)
                        else:
                            raise RuntimeError(
                    ...
                    model_state_dict.update(motion_state_dict)
                    # load the weights into the model
                    m, u = self.unet.load_state_dict(model_state_dict, strict=False)
                ...
                # print(f"nosie_pred shape is{noise_pred.shape}")
                counter = torch.zeros(
                ...
                for context in context_scheduler(
                    ...
                    for layer_index in range(0, latent_model_input.shape[0], unet_batch_size):
                        ...
                        pred_layer = self.unet(
                            ...
                            t,
                            #encoder_hidden_states=prompt_embeds_list[cur_i],
